...
/feature <request text...> [startAt=discover|impl] [bead=<id>] [brief=<text>]
         ...
         [skipReview=true] [autoApprove=true] [model=<tier|id>] [models=phase:tier,...]
```

Run-1: `/feature add rate limiting to the ingest API`
Run-2 (after human approves design): `/feature startAt=impl bead=<id>`
One-shot (skip the human seam): `/feature autoApprove=true add rate limiting...` — design->impl->ci->review->testing in a single invocation.
...
| `autoApprove` | `true` skips the design->impl human handoff: after the design is APPROVED, run-1 falls through directly into run-2 (impl->ci->review->testing) in the same invocation, reloading the just-persisted design from the bead. Omit for the default human-approval seam. | false (human seam) |
...
Recognized by the parser's `CONTROL_KEYS` but NOT consumed by this workflow: `depth`, `mode`, `maxIterations`, `window`, `pr`. (`pr` only influences bead-id derivation in `runBeadId`.) Because feature's CI loop is not given a PR number, it verifies against the **local test gate** (`npm test`) rather than watching remote checks — see [docs/ci-cd-integration.md](../ci-cd-integration.md).
...
## Profiles: `profile=full|small`

`profile=full` (default) runs the complete lifecycle (research -> discover ->
design -> impl -> CI -> review -> testing). Behavior is unchanged from prior
releases.

`profile=small` runs a lean lifecycle that **skips the Design panel and Review
council** (research -> discover -> impl -> CI -> testing), for small, low-risk
additive changes whose intent is already clear. It replaces the former
standalone `/small-feature` workflow.

- `startAt=design` is rejected under `profile=small` (no Design phase).
- Other `startAt` values, `bead=`, and `brief=` behave as in `profile=full`.
- For bugs use `/debug` (reproduce -> root-cause -> fix), not `profile=small`.

Example: `/feature profile=small brief=Add a --json flag to the status command`
