...
- **Backtrack (gate recovery)**: if Refactor exhausts its budget + escalation, refactor re-runs the prior Research (blast-radius mapping) phase once per `PHASE_BUDGETS.backtrack` (default `0` = off) with the refactor-failure feedback folded in, before `needs_human` — for when the refactor keeps failing because call sites were under-mapped. Default-off = behavior unchanged. See `docs/designs/2026-06-16-backtrack-gate-recovery.md` + the standardization policy in `docs/architecture.md`.
