...
| `perspectives` | Comma list of perspective agents to fan out. Filtered by `validPerspectives` (defaults + opt-in `persona`, `repo-conventions`); empty/invalid falls back to the default 6. | the 6 defaults | yes (read directly) |
...
- `standard`: + security, scope-alignment, error-handling, api-contract, simplification
- `full`: + performance, deployment-risk, maintainability (simplification already included at standard)
...
All perspective agents fan out in parallel during Perspectives; the arbiter runs alone in Synthesis. The model tier below is what the workflow assigns at spawn time via `modelFor`, which overrides each agent file's own `model:` frontmatter.
...
| `simplify` | Perspectives | Finds reuse misses, unrequested abstraction, dead code, altitude mismatches; quality-only, findings not fixes | `review` -> mid (claude-sonnet-4-6) |
...
- `depth-map` (src/fragments/depth-map.js): `REVIEW_DEPTHS`, `DEFAULT_PERSPECTIVES` (advocate, critic, security, performance, learning, simplify), `validPerspectives()`, `criteriaForDepth()`.
