...
**CI parity (GitLab):** `ci-loop.js` is VCS-aware. The CI-watch agent prompt instructs it to detect the host from `git remote get-url origin` and use `gh pr checks --watch` for GitHub or `glab ci status`/`glab pipeline status` for GitLab. This applies to all three callers of `runCI` (feature, small-feature, finish-pr).
