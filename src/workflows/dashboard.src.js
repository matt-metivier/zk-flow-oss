...
// @@USE: handoff,schemas,args,bd-memory,bead-run,model-tiers,env-check,operating-posture,skill-render,context-pack
export const meta = {
  name: 'dashboard',
...
// Observability skills (grafana/monitoring/observability-stack) for the edit phase.
const skillsBlock = await selectAndRenderSkills(
  'grafana dashboard edit: ' + (a.brief || '') + ' api=' + (a.api || ''),
  null,
  modelFor('discover', a)
);

// Durable context: machine persona + prior beads + vault MOC (no discover phase here).
const ctxBlock = await contextPack('dashboard edit: ' + (a.brief || ''), 'observability dashboard', modelFor('discover', a));
...
Emit {"applied":true,"summary":"<what changed or already-present no-op>"} or {"applied":false,"summary":"<error>"}.${skillsBlock}${ctxBlock}`,
