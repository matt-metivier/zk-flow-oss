...
phase('Draft');
let design = await agent(`${postureFor('design', a)}\n\nDraft the SQCA design. ${ideationNote}${a.brief || ''}`, { label:'designer', phase:'Draft', agentType:'designer', schema: SCHEMAS.design, model: modelFor('design', a) });
...
for (let di = 1; di <= PHASE_BUDGETS.design; di++) {
  ...
  const persp = await parallel(validPerspectives(DEFAULT_PERSPECTIVES).map(p => () =>
    agent(`${postureFor('review', a)}\n\nReview the design from the "${p}" perspective: ${JSON.stringify(design)}`, { label:`design:${p}:${di}`, phase:'Council', agentType:p, model: modelFor('review', a) })));
  grade = await agent(
