...
export async function runPhase({ phasePrompt, phaseSchema, agentType, label, maxIterations, gradePrompt, model, gradeModel }) {
  let out, grade, feedback = '';
  for (let i = 1; i <= maxIterations; i++) {
    out = await agent(phasePrompt(i, feedback), { schema: phaseSchema, agentType, label: `${label}:${i}`, ...(model !== undefined && { model }) });
