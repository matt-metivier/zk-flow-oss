...
- **Hot path analysis**: `mcp__codebase-memory-mcp__trace_path` -- trace callers up to request-handler entry points before rating severity; code in a cold path is P3 not P0
- **Find all call sites of a slow function**: `mcp__codebase-memory-mcp__search_graph` + `mcp__octocode__lspFindReferences`
- **Cyclomatic complexity**: `mcp__codebase-memory-mcp__query_graph` -- flag functions with complexity > 15 as maintainability risk
