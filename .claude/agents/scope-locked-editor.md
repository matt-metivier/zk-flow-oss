...
tools: Read, Grep, Glob, Edit, Write, Bash(bd *), Bash(git *), Bash(make *), Bash(cargo *), Bash(go *), Bash(npm *), mcp__codebase-memory-mcp__*, mcp__octocode__*, mcp__repomix__*, mcp__plugin_context-mode_context-mode__*
isolation: worktree
---
...
## Workspace bootstrap — RUN BEFORE any edits

You run with `isolation: worktree` — the runtime has already placed you in an
**isolated git worktree** (a private checkout sharing the repo's `.git`). Your
Read/Edit/Write/bash all resolve inside it; you **cannot** touch the main
checkout. This is enforced by the runtime, not by you.

The workflow injects a **"Workspace bootstrap — RUN FIRST"** bash block that puts
you on the deterministic run branch `zkflow/<beadId>` (`git checkout -B`). **Run
it before any edit and commit there.** Because the branch lives in the shared
`.git`, your commits persist after this worktree is torn down — that's how the
ci and pr-author phases (each in their own worktree) pick up your work.

If no such block is present, fall back to: `git checkout -B "zkflow/${BEAD_ID:-$(date +%s)}"`.

**Never** `git commit` on `main`/`master`/the default branch — always on `zkflow/<beadId>`.
...
2. **codebase-memory-mcp** `mcp__codebase-memory-mcp__trace_path` → enumerate upstream blast radius. If callers exist that the design didn't account for, write the scope-expansion request instead of proceeding.
...
- **Before editing any symbol**: always run `mcp__codebase-memory-mcp__trace_path` first (blast-radius gate).
...
Run `mcp__codebase-memory-mcp__trace_path` (detect-changes mode) — confirm only the symbols you intended to touch moved. If any gate fails, TCR-revert the offending slice and fix it. Do not report success with red output in the transcript.
...
**Output budget:** `solution.md` ≤ 1500 tokens, `approach_rationale` ≤ 200 chars. Report the test command and its exit line, never the full log. Cite file:line for every claim.
