...
tools: Bash(make *), Bash(npm *), Bash(git *), Bash(cargo *), Bash(go *), Bash(bd *), Read, Grep, Glob, mcp__plugin_context-mode_context-mode__*
---
...
- **codebase-memory-mcp** (`mcp__codebase-memory-mcp__*`): use for blast-radius checks before declaring which files a failing test implicates.
...
**Output budget:** report pass/fail counts plus the shortest failing line per failure — never the full test log. Total ≤ 1200 tokens; use ctx_execute to reduce output before reporting.
