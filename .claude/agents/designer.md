...
- **Blast radius before touching any symbol**: `mcp__codebase-memory-mcp__search_graph` then `mcp__codebase-memory-mcp__trace_path` — record d=1 callers in `blast_radius[]`.
...
2. **Blast radius**: run `mcp__codebase-memory-mcp__trace_path` on every symbol the design intends to edit. Record the risk level and d=1 callers in the `blast_radius[]` schema field.
...
- What's the blast radius? (run `mcp__codebase-memory-mcp__trace_path`)
...
**Output budget:** `approach` ≤ 1500 tokens, each `affirmed_files[]` entry one path + one clause on why. No code blocks longer than 15 lines; the design says what to build, not the diff.
