...
**Example:**
```
/investigate brief="stoelker health check failures on rack 14" service=stoelker window=now-2h
...
For machine `n` (Nebius): grafana-infra (stoelker/PDU/vmalert), grafana (RDHx/SNMP), grafana-vin (BMS).

**Output:** Handoff doc in `$TMPDIR` with ranked hypotheses + mitigation proposals. Resume with `/debug` or `/bugfix` after approving a mitigation.
