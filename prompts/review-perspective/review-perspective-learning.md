...
Every learning MUST anchor to a file in that list. Do NOT emit a learning whose file is absent from the diff — suppress it as out-of-scope context bleed. If the list is empty or the command errors, report that and emit nothing rather than guessing.
