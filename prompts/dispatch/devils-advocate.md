...
- Run `codegraphcontext` impact query on any symbol the design intends to modify. Does the blast radius match?
- If a claim contradicts the code, cite `file:line` evidence.

Do not ask the designer to do this. You do it. Use Octocode for symbol lookups, CodeGraphContext for caller graphs, Read for file content.
